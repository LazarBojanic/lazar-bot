...
function registerMethods(client, botChannel) {
    client.on('message', (channel, tags, message, self) => {
        ...
        if (chatMessage === '!newGame') {
            fetch(`${SERVER_IP}game/new-game?username=${chatterUsername}`)
                .then(res => res.json())
                .then(res => {
            ...
            fetch(`${SERVER_IP}game/guess`, {
                ...
                .then(res => {
                    ...
                    if (res.reason === 'gameEnded') {
                        client.say(channel, 'Game ended. You can start a new one with !newGame');
                    }
                    else if (res.reason === 'solutionNotValid') {
                        client.say(channel, `Guess: ${guessWord}, not valid!`);
                    }
                    else {
                        let letterStatusesEmojis = '';
                        for (let i = 0; i < res.validatedWord.letters.length; i++) {
                            switch (res.validatedWord.letters[i].status) {
                                case 'R':
                        ...
                        fetch(`${SERVER_IP}game/check-status?username=${chatterUsername}`, {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json'
                            }
