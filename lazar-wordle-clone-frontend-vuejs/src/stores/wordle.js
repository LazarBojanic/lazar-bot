...
export const useWordleStore = defineStore('wordle', () => {
  ...
  async function submitWord(){
    if (currentCol.value >= 5){
      const guess = currentWord.value.characterTiles.map(t => t.character).join('');
  ...
  return { board, currentRow, currentCol, currentWord, keyboard, keyBoardRows, addLetter, deleteLetter, submitWord };
});
